...
Rails.application.configure do
  ...
  config.server_timing = ENV.fetch('SERVER_TIMING', true) == 'true'
  ...
  if Rails.root.join('tmp', 'caching-dev.txt').exist?
    ...
    config.action_controller.enable_fragment_cache_logging = ENV.fetch('ACTION_CONTROLLER_ENABLE_FRAGMENT_CACHE_LOGGING', true) == 'true'
  ...
  config.active_record.verbose_query_logs = ENV.fetch('ACTIVE_RECORD_VERBOSE_QUERY_LOGS', true) == 'true'

  # Debug mode disables concatenation and preprocessing of assets.
  # This option may cause significant delays in view rendering with a large
  # number of complex assets.
  config.assets.debug = ENV.fetch('ASSETS_DEBUG', false) == 'true'

  # Suppress logger output for asset requests.
  config.assets.quiet = ENV.fetch('ASSETS_QUIET', true) == 'true'
  ...
  # config.action_cable.disable_request_forgery_protection = ENV.fetch('ACTION_CABLE_DISABLE_REQUEST_FORGERY_PROTECTION', true) == 'true'
