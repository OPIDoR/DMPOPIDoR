...
namespace :upgrade do
  ...
  task normalize_language_formats: :environment do
    Language.all.each do |language|
      val = LocaleService.to_i18n(locale: language.abbreviation).to_s
      language.update(abbreviation: val)
    end
    Template.all.each do |template|
      next if template.locale.blank?

      template.update(locale: LocaleService.to_i18n(locale: template.locale).to_s)
    end
    Theme.all.each do |theme|
      next if theme.locale.blank?

      theme.update(locale: LocaleService.to_i18n(locale: theme.locale).to_s)
    end
  end
  ...
  task migrate_plan_org_and_funder: :environment do
    ...
    Parallel.map(plans, in_threads: 8) do |plan|
      ...
      plan.update_columns(org_id: plan.owner.org.id)
    end
    ...
    Parallel.map(plans, in_threads: 8) do |plan|
      ...
      plan.update_columns(funder_id: funder_id) if funder_id.present?
    end
    p 'Complete'
  end
  ...
  task migrate_plan_grants: :environment do
    ...
    plans.each do |plan|
      ...
      plan.update_columns(grant_id: identifier.id)
    end
    p 'Complete'
  end
