...
class MadmpFragment < ApplicationRecord
  ...
  def self.fragment_exists?(data, schema, dmp_id, parent_id = nil, current_fragment_id = nil)
    ...
    filtered_incoming_data = data.to_h.delete_if { |_, v| v.nil? || v.to_s.empty? }.slice(*unicity_properties)

    dmp_fragments.each do |fragment|
      filtered_db_data = fragment.data.delete_if { |_, v| v.nil? || v.to_s.empty? }.slice(*unicity_properties)
