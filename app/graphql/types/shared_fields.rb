  ...
  module SharedFields
    def self.shared_and_or_filter(level: 0)
      Proc.new do
        ...
        argument :filter, -> { Types::LogicalFilterInput }, required: false, description: 'A nested filter to allow complex filtering conditions.' if level == 0
        argument :filter, -> { Types::LogicalSubfilterInput }, required: false, description: 'A nested filter to allow complex filtering conditions.' if level == 1
      end
    end
