...
json.templates @org_templates.each(org, templates {
  json.organisation_name    org.name
  json.organisation_id      org.id
  json.is_funder            org.funder?
  json.organisation_templates templates[:own].each(_, template {
    json.title              template.title
    json.id                 template.family_id
    json.description        template.description
  })
  json.customized_templates templates[:cust].each(_, template {
    json.title              template.title
    json.id                 template.family_id
