...
describe("FragmentList component", () => {
  test("component rendering without lines", async () => {
    ...
    expect(
      screen.queryByTestId(/fragment-list-row-[0-9]+/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("fragment-list-table-header"),
    ).not.toBeInTheDocument();
  });
  ...
  test("component rendering with lines", async () => {
    ...
    expect(screen.getByTestId("fragment-list-table")).toBeInTheDocument();
    expect(
      screen.getByTestId("fragment-list-table-header"),
    ).toBeInTheDocument();
    expect(screen.getByTestId("fragment-list-table-header")).toHaveTextContent(
      fragmentListProps.tableHeader,
    );
    expect(screen.getByTestId("fragment-list-table-header")).toHaveTextContent(
      "Actions",
    );
    expect(screen.getByTestId("fragment-list-table-body")).toBeInTheDocument();
    expect(screen.queryAllByTestId(/fragment-list-row-[0-9]+/i)).toHaveLength(
      data.length,
    );
    expect(
      screen.queryAllByTestId(/fragment-list-row-edit-btn-[0-9]+/i),
    ).toHaveLength(data.length);
    expect(
      screen.queryAllByTestId(/fragment-list-row-delete-btn-[0-9]+/i),
    ).toHaveLength(data.length);
    expect(
      screen.queryAllByTestId(/fragment-list-row-show-btn-[0-9]+/i),
    ).toHaveLength(0);
  });
  ...
  test("component not rendering fragment with action=delete", async () => {
    const fragmentListWithDeleteProps = {
      ...
      fragmentsList: [
        { title: "First Line" },
        { title: "Second Line", action: "delete" },
      ],
    };
  ...
  test("component should render as readonly", async () => {
    ...
    expect(
      screen.queryByTestId(/fragment-list-row-delete-btn-[0-9]+/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId(/fragment-list-row-edit-btn-[0-9]+/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryAllByTestId(/fragment-list-row-show-btn-[0-9]+/i),
    ).toHaveLength(2);
  ...
  test("component should render fragment value according to templateToString", async () => {
    ...
    expect(screen.getByTestId("fragment-list-row-value-0")).toHaveTextContent(
      "myName ( test@test.fr )",
    );
  });
});
