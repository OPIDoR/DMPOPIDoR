...
describe("PersonsList component", () => {
  test("component rendering without lines", async () => {
    ...
    expect(
      screen.queryByTestId(/persons-list-row-[0-9]+/i),
  ...
  test("component rendering with lines", async () => {
    ...
    expect(
      screen.queryAllByTestId(/persons-list-row-edit-btn-[0-9]+/i),
    ).toHaveLength(data.length);
    expect(
      screen.queryAllByTestId(/persons-list-row-delete-btn-[0-9]+/i),
    ).toHaveLength(data.length);
    expect(
      screen.queryAllByTestId(/persons-list-row-show-btn-[0-9]+/i),
    ).toHaveLength(0);
  });
  ...
  test("component rendering default role as a string when role is constant", async () => {
    const personsListWithConstantRoleProps = {
      ...personsListProps,
    ...
    expect(screen.getByTestId("persons-list-role-0")).toHaveTextContent(
      personsListWithConstantRoleProps.defaultRole,
    );
  });
});
