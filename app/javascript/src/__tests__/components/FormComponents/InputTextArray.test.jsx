...
describe("InputTextArray component", () => {
  test("component rendering", async () => {
    ...
    expect(screen.getByTestId("input-text-array-label")).toHaveTextContent(
      inputTextArrayProps.label,
    );
    expect(screen.getByTestId(/custom_button_[0-9]+/i)).toBeInTheDocument();
    expect(
      screen.queryByTestId(/input-text-array-div-[0-9]+/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId(/input-text-array-[0-9]+/i),
    ).not.toBeInTheDocument();
  ...
  test("component rendering input when clicking on add button", async () => {
    ...
    expect(
      screen.queryByTestId("input-text-array-div-0"),
    ).not.toBeInTheDocument();
  ...
  test("component removing input when clicking on delete button", async () => {
    ...
    expect(
      screen.queryAllByTestId(/input-text-array-div-[0-9]+/i),
  ...
  test("component rendering as readonly", async () => {
    const inputTextArrayReadonlyProps = {
      ...inputTextArrayProps,
      readonly: true,
    };
    render(
      <Wrapper
        propName={inputTextArrayReadonlyProps.propName}
        data={["myText1"]}
      >
    ...
    expect(screen.getByTestId("input-text-array-0")).toBeInTheDocument();
    expect(screen.getByTestId("input-text-array-0").value).toBe("myText1");
    expect(screen.getByTestId("input-text-array-0")).toHaveAttribute(
      "readonly",
    );
    expect(
      screen.queryByTestId("input-text-array-delete-icon-0"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId(/custom_button_[0-9]+/i),
    ).not.toBeInTheDocument();
  });
});
