...
import { StyledUl, StyledLi, GlossaryContent } from "./styles.jsx";
...
export default function HelpPage({ locale, directusUrl }) {
    ...
    <StyledUl>
      {Object.keys(letters).map((key, index) => (
        <StyledLi
          ...
          onClick={() =>
            Object.entries(letters[key]).length === 0 ? null : scrollToId(key)
          }
  ...
  const displayTerm = (termData, index) => (
    <li key={`glossary-term-${termData.at(0).toLowerCase()}`}>
      <div className="term" key={`glossary-term-${index}`}>
        {termData.at(0)}
      </div>
      <div
      ...
      <GlossaryContent>
        {Object.keys(letters).map((letter, index) => (
          <ul id={`glossary-letter-${letter}`} key={letter}>
            <div
              className={`letter ${activeLetter === letter ? "active" : null}`}
            >
              {letter}
              <span>
            ...
            {Object.entries(letters[letter]).length > 0 &&
              Object.entries(letters[letter]).map((termData) =>
                displayTerm(termData, index),
              )}
          </ul>
        ))}
      </GlossaryContent>
