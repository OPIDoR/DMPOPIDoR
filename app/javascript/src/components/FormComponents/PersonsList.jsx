    ...
    <>
      {personsList && (
        <table
          ...
          className="table"
          data-testid="persons-list-table"
        >
          ...
          <tbody data-testid="persons-list-table-body">
            {personsList.map((el, idx) =>
              el.action !== "delete" ? (
                <tr key={idx} data-testid={`persons-list-row-${idx}`}>
                  <td
                    style={{ width: "50%" }}
                    data-testid={`persons-list-row-value-${idx}`}
                  >
                    <div className={styles.cell_content}>
                      <div>
                        {parsePattern(
                          el.person,
                          templateToString.length > 0
                            ? templateToString
                            : ["$.lastName", " ", "$.firstName"],
                        )}{" "}
                      </div>
                  ...
                  <td
                    ...
                    data-testid={`persons-list-role-${idx}`}
                  >
                    {isRoleConst && defaultRole}
                    {roleOptions && !isRoleConst && (
                      <CustomSelect
                        onSelectChange={(e) => handleSelectRole(e, idx)}
                        options={roleOptions}
                        selectedOption={{
                          label: el.role || defaultRole,
                          value: el.role || defaultRole,
                        }}
                        name="role"
                        isDisabled={readonly}
