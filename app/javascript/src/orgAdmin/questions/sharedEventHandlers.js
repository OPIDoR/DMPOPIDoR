const onChangeQuestionFormat = (e) => {
  ...
  const defaultValue = $(source)
    .closest("form")
    .find('[data-attribute="default_value"]');
  const questionOptions = $(source)
    .closest("form")
    .find('[data-attribute="question_options"]');
  const opComment = $(source)
    .closest("form")
    .find('[data-attribute="option_comment"]');
  switch (selected) {
    case "1":
...
export const onChangeQuestionClassname = (e) => {
  ...
  $(source)
    .closest("form")
    .find(`select.question_schema option[data-classname="${selected}"]:first`)
    .prop("selected", true);
  $(source)
    .closest("form")
    .find(`select.question_schema option[data-classname!="${selected}"]`)
    .hide();
};
