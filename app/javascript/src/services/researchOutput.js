...
const create = async (jsonObject) =>
  axios.post("/research_outputs", jsonObject, {
    headers: createHeaders({}, true),
  });

const update = async (id, jsonObject) =>
  axios.patch(`/research_outputs/${id}`, jsonObject, {
    headers: createHeaders({}, true),
  });
...
const importResearchOutput = async ({ planId, uuid, duplicate }) => {
  ...
  try {
    res = await axios.post(
      `/research_outputs/import?plan_id=${planId}`,
      { uuid, duplicate },
      { headers: createHeaders({}, true) },
    );
  } catch (error) {
...
const deleteResearchOutput = async (researchOutputId) => {
  let response;
  try {
    response = await axios.delete(`/research_outputs/${researchOutputId}`, {
      headers: createHeaders({}, true),
    });
  } catch (error) {
