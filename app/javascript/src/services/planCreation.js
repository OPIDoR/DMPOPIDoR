 ...
 */
const getRecommendedTemplate = async (researchContext, locale) =>
  axios.get(
    `/template_options/recommend?context=${researchContext}&locale=${locale}`,
  );
 ...
 */
const getOrgs = async (researchContext, templateLanguage) =>
  axios.get(
    `/orgs/list?context=${researchContext}&locale=${templateLanguage}&type=org`,
  );
 ...
 * @returns
 */
const createPlan = async (template_id, context) =>
  axios.post(
    "/plans",
    { template_id, context },
...
const importPlan = async (formData) =>
  axios.post("/plans/import", formData, {
    headers: {
      ...createHeaders({}, true),
