import { isObject, isNumber, isString } from "../utils/isType";
...
const showSavingMessage = (jQuery) =>
  jQuery.closest(".question-form").find('[data-status="saving"]').show();
const hideSavingMessage = (jQuery) =>
  jQuery.closest(".question-form").find('[data-status="saving"]').hide();
const showLoadingOverlay = (jQuery) => jQuery.find(".overlay").show();
const hideLoadingOverlay = (jQuery) => jQuery.find(".overlay").hide();
const closestErrorSavingMessage = (jQuery) =>
  jQuery.closest(".question-form").find('[data-status="error-saving"]');
const questionId = (jQuery) =>
  jQuery.closest(".form-answer").attr("data-autosave");
...
export const doneCallback = (data, jQuery) => {
  ...
  if (isObject(data)) {
    if (isObject(data.question)) {
      // Object related to question within data received
      if (isNumber(data.question.id)) {
        if (isString(data.question.answer_status)) {
          $(
            `#answer-status-${data.question.id}-research-output-${data.research_output.id}`,
          ).html(data.question.answer_status);
        ...
        if (isString(data.question.locking)) {
          ...
          $(
            `#answer-form-${data.question.id}-research-output-${data.research_output.id}`,
          ).html(data.question.form);
          // Retrieves the newly form added to the DOM
          const newForm = $(
            `#answer-form-${data.question.id}-research-output-${data.research_output.id}`,
          ).find("form");
          // Attaches event handlers for the new form
          attachEventHandlers(newForm);
          // Refresh optimistic locking view with the form that caused the locking
          $(
            `#answer-locking-${data.question.id}-research-output-${data.research_output.id}`,
          ).html(data.question.locking);
        } else {
          // When answer is NOT stale...
          $(
            `#answer-locking-${data.question.id}-research-output-${data.research_output.id}`,
          ).html("");
          detachEventHandlers(form);
          ...
          if (isNumber(data.question.answer_lock_version)) {
            newForm
              .find("#answer_lock_version")
              .val(data.question.answer_lock_version);
          }
        }
      }
    } // End Object related to question within data received
    if (isObject(data.plan)) {
      // Object related to plan within data received
...
export const failCallback = (error, jQuery) => {
  closestErrorSavingMessage(jQuery)
    .html(
...
document.addEventListener("turbo:load", () => {
  ...
  $('.research-outputs-tab a[data-toggle="tab"]').on("shown.bs.tab", (e) => {
    ...
    const tabsList = $(
      `.research-outputs-tab a[data-research-output="${researchOutputId}"]`,
    );
  ...
  $(document).on("click", ".toggle-guidance-section:not(.disabled)", (e) => {
    ...
    target
      .find("span.fa-chevron-right, span.fa-chevron-left")
