...
document.addEventListener("turbo:load", () => {
  ...
  const sensitiveInfoCheck = (event) => {
    ...
    const email = $(
      '#personal_details_registration_form [name="user[email]"]',
    ).val();
    const org = $("#personal_details_registration_form #user_org_id").val();
    const pwd = $(
      '#password-confirmation input[name="user[current_password]"]',
    ).val();
    ...
    if (isString(originalEmail) && isString(email)) {
      if (
        originalEmail.toLowerCase() !== email.toLowerCase() &&
        !isValidPassword(pwd)
      ) {
  ...
  $('#password-confirmation input[name="user[current_password]"]').on(
    "change",
    (e) => {
      $('#password-confirmation input[name="user[password]"]').val(
        $(e.target).val(),
      );
    },
  );
