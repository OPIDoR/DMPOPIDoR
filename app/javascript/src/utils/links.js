...
document.addEventListener("turbo:load", () => {
  ...
  const replacerFor = (i, el) =>
    $(el).attr("for", $(el).attr("for").replace(regExp, replacer));
  const replacerId = (i, el) =>
    $(el).attr("id", $(el).attr("id").replace(regExp, replacer));
  ...
  const linksLength = (jqueryElem) =>
    jqueryElem.closest(".links").find(".link").length;
  const maxNumberLinks = (jqueryElem) =>
    jqueryElem.closest(".links").attr("data-max-number-links") * 1;
