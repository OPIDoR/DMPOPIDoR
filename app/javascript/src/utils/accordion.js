document.addEventListener("turbo:load", () =>
  $("body").on("click", ".accordion-controls a[data-toggle-direction]", (e) => {
    ...
    if (direction) {
      ...
      const parentTarget = $(`#${parentTargetName}`).length
        ? $(`#${parentTargetName}`)
        : $(`.${parentTargetName}`);
      parentTarget.find(".accordion-item").each((i, el) => {
        ...
        let bsCollapse =
          window.bootstrap.Collapse.getInstance(accordionCollapse);
        if (!bsCollapse) {
          bsCollapse = new window.bootstrap.Collapse(accordionCollapse, {
            toggle: false,
          });
        }
